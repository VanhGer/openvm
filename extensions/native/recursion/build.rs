...
fn main() {
    ...
    // Copy ffi.h to OUT_DIR/ffi.h
    let header_src = PathBuf::from("ffi.h");
    let header_dest = dest_path.join("ffi.h");
    std::fs::copy(header_src, header_dest).unwrap();

    // Copy libsp1gnark.a to OUT_DIR/libsp1gnark.a
    let lib_src = PathBuf::from("libsp1gnark.a");
    let lib_dest = dest_path.join("libsp1gnark.a");
